...
useEffect(() => {
  const fetchOrders = async () => {
    try {
      ...
      // 1️⃣ هات أول عميل بنفس رقم الهاتف
      const { data: customer, error: customerError } = await supabase
        .from("customers")
        .select("id")
        .eq("phone", userDetails.phone)
        .limit(1)
        .single();
      ...
      // 2️⃣ هات كل الطلبات المرتبطة بهذا العميل
      const { data: ordersData, error: ordersError } = await supabase
        .from("orders")
        .select(
          `id, items, total, status, created_at, customers(name, phone, location)`
        )
          ...
          orders.map((order) => (
            ...
            >
              ...
              <p className="font-bold text-right pr-5">
                {" "}
                Total: <span className="text-amber-500">
                  {order.total}$
                </span>{" "}
              </p>{" "}
            </div>
